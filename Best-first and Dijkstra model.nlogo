globals [
  ...
  nodes_BFs  ; List used by best first search
  end_BFs ; Variable used to store the end-Point
  d_Cost   ; number of patches visited dijkstra
  bf_Cost  ; as above best first
  d_found  ; boolean in case no possible path dijkstra
  bf_found ; as above best first
]
...
to paint-both [x c] ; Help function to draw in both mazes the same
  set pcolor c
  ifelse x < 0
  [ set x (33 + x) ]
  [ set x (x - 33) ]
...
to clear ; All the neighbors and the path painted is erased and clean global variables
  ...
  set nodes_BFs lput patches with [pcolor = green and pxcor < 0] nodes_BFs ; as above for best first
...
to go
  clear
  while [ length nodes_Dj != 0 or length nodes_BFs != 0] ; Run while some algorithm is stil running
  [
...
to BestFirst
  ...
  let current-patch item 0 nodes_BFs                               ; Get the next neighborh path to process
