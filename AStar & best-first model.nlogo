globals [
  nodes_BFs  ; List used by Best First
  nodes_As   ; List used by A*
  end_As     ; Variable used to store the end-Point for A*
  end_BFs    ; Variable used to store the end-Point for Best first
  bf_Cost    ; number of patches visited best first
  a_Cost     ; as above for A*
  bf_found   ; boolean in case no possible path best first
...
to clear ; All the neighbors and the path painted is erased and clean global variables
  ...
  set nodes_BFs []
  set nodes_As []
  set nodes_BFs lput patches with [pcolor = green and pxcor > 0] nodes_BFs  ; start point first for best first
  set nodes_As lput patches with [pcolor = green and pxcor < 0] nodes_As    ; as above for A*
  set end_BFs one-of patches with [pcolor = red and pxcor > 0]              ; end point needed for best first calculations
  set end_As one-of patches with [pcolor = red and pxcor < 0]               ; as above for A*
...
to go
  clear
  while [ length nodes_BFs != 0 or length nodes_As != 0]                    ; Run while some algorithm is stil running
  [
    if length nodes_BFs > 0 [BestFirst]
    if length nodes_As > 0 [A*]
    wait delay
    tick
  ]
  if not bf_Found[output-print "Best-First Route not found"]
...
to BestFirst
  set nodes_BFs sort-by [ [a b] -> [d] of a < [d] of b ] nodes_BFs        ; Sort the list to find the shortest patch
  let current-patch item 0 nodes_BFs                                      ; Get the next neighbour path to process
  set nodes_BFs remove-item 0 nodes_BFs                                   ; Remove from the list
  ask current-patch
  [
    if pcolor = violet[
      set pcolor yellow
    ...
    ifelse pcolor = red                                                   ; If the current-patch is the end-point:
    [
      set nodes_BFs [] ; Clear the list
      paint-path "Best First " bf_Cost              ; Paint the path and show the proccess time, cost and route length
      set bf_Found true
    ]
    [
      ; If is not the end-point continue searching with the neighbors
      ask neighbors4 with [pcolor = white or pcolor = red]
      [
        set bf_Cost bf_Cost + 1
        set d distance end_BFs                            ; Calculate the distance between the current neighbor to the end point
        paint-neighbour                                   ; mark as neighbour
        set nodes_BFs lput self nodes_BFs                 ; Add the neighbor to the list
      ]
    ]
    if pcolor != green and pcolor != red[
      set pcolor violet
...
;returns the cost of the path so far plus the heuristic distance to goal
